...
public final class InputStream extends Stream {
   private static final int[] BIT_MASK = new int[]{0, 1, 3, 7, 15, 31, 63, 127, 255, 511, 1023, 2047, 4095, 8191, 16383, 32767, 65535, 131071, 262143, 524287, 1048575, 2097151, 4194303, 8388607, 16777215, 33554431, 67108863, 134217727, 268435455, 536870911, 1073741823, Integer.MAX_VALUE, -1};
   ...
   public void finishBitAccess() {
      this.setOffset((7 + this.bitPosition) / 8);
   }
   ...
   public void checkCapacity(int length) {
      if (this.getOffset() + length >= this.buffer.length) {
         byte[] newBuffer = new byte[(this.getOffset() + length) * 2];
   ...
   public int getRemaining() {
      return this.getOffset() < this.length ? this.length - this.getOffset() : 0;
   }

   public void addBytes(byte[] b, int offset, int length) {
      this.checkCapacity(length - offset);
      System.arraycopy(b, offset, this.buffer, this.getOffset(), length);
   ...
   public int readSmart2() {
      ...
      int i_33_;
      for(i_33_ = this.readUnsignedSmart(); i_33_ == 32767; i += 32767) {
   ...
   public int readBigSmart() {
      if (~this.buffer[this.getOffset()] <= -1) {
   ...
   public int readUnsignedSmart() {
      int i = 255 & this.buffer[this.getOffset()];
