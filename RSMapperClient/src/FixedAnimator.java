public class FixedAnimator extends Animator {
   ...
   static final void getItemsForGrandExchangeSearch(boolean bool, byte b, boolean showAll, int i, int i_1_, String string, String string_2_) {
      ...
      for(int itemId = 0; EntityNode_Sub3_Sub1.aClass86_9166.anInt1175 > itemId; ++itemId) {
         ...
         if ((!showAll || itemdefinition.showInGrandExchange)
            && itemdefinition.certTemplateId == -1
            && itemdefinition.lendTemplateId == -1
            && ~itemdefinition.unknownValue1 == 0
            && itemdefinition.dummyItem == 0
            && itemdefinition.name.toLowerCase().indexOf(string) != -1
            && itemdefinition.canExchange(itemId)
            && (
               ~i_1_ == 0
                  || (!bool ? ~i == ~itemdefinition.method1678(i_1_, i_3_, 10247) : string_2_.equals(itemdefinition.method1684(string_4_, b ^ 32573, i_1_)))
            )) {
            if (~i_5_ <= -251) {
