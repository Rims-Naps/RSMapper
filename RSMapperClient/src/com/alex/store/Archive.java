...
public class Archive {
   ...
   private void decompress(byte[] archive) {
      ...
      if (compressedLength >= 0 && compressedLength <= 1000000000) {
         switch(this.compression) {
            case 0:
   ...
   public Object[] editNoRevision(byte[] data, com.alex.store.MainFile mainFile) {
      this.data = data;
      ...
      return !mainFile.putArchiveData(this.id, compressed)
         ? null
         : new Object[]{CRC32HGenerator.getHash(compressed), Whirlpool.getHash(compressed, 0, compressed.length)};
   }
